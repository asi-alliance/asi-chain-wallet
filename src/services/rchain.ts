...
const globalBalanceCache: Map<string, { balance: string; timestamp: number }> =
    new Map();
...
export class RChainService {
    ...
    constructor(
        ...
        if (!nodeUrl || !nodeUrl.trim()) {
            if (!graphqlUrl || !graphqlUrl.trim()) {
                throw new Error(
                    "RChainService: either nodeUrl (validator URL) or graphqlUrl must be provided",
                );
            }
            this.nodeUrl = "";
        } else {
        ...
        this.graphqlUrl =
            ...
            "http://18.142.221.192:8080/v1/graphql";
    ...
    private async rnodeHttp(apiMethod: string, data?: any): Promise<any> {
        ...
        if (apiMethod === "propose" && this.adminClient) {
            ...
            apiMethod === "explore-deploy" ||
            (this.isReadOnlyOperation(apiMethod) && !isPost)
        ) {
        ...
        try {
            ...
            const isExploreDeployString =
                apiMethod === "explore-deploy" && typeof data === "string";
            ...
            if (error.response) {
                throw new Error(
                    `RNode API Error: ${error.response.status} - ${JSON.stringify(error.response.data)}`,
                );
    ...
    async getBalance(
        ...
        if (
            !forceRefresh &&
            cached &&
            now - cached.timestamp < BALANCE_CACHE_TTL
        ) {
        ...
        try {
            ...
            if (result && result.length > 0) {
                ...
                if (firstExpr?.ExprInt?.data !== undefined) {
                    ...
                    globalBalanceCache.set(cacheKey, {
                        balance,
                        timestamp: now,
                    });
                    return balance;
        ...
        try {
            ...
            try {
                ...
                if (
                    ...
                    throw new Error(
                        `Deploy failed: Server is unreachable. The deploy was NOT sent to the network. Please check your connection and try again.`,
                    );
                }
    ...
    async waitForDeployResult(
        ...
        if (isMixedContent) {
            console.warn(
                `[GraphQL] Mixed-content detected: page is HTTPS but GraphQL is HTTP. Skipping GraphQL query and using fallback method.`,
            );
  ...
  deployments(where: {deploy_id: {_eq: $deployId}}) {
    ...
    block {
      ...
      proposer
                ...
                try {
                    ...
                    console.error(
                        `[GraphQL] Request failed for deploy ${deployId}:`,
                        {
                    ...
                    if (
                        ...
                        (error.code === "ERR_NETWORK" &&
                            typeof window !== "undefined" &&
                            window.location.protocol === "https:" &&
                            graphqlEndpoint.startsWith("https://"))
                    ) {
                        console.warn(
                            `[GraphQL] Mixed-content or HTTPS error detected. Using fallback method.`,
                        );
                        return this.waitForDeployResultFallback(deployId);
                    }
                    ...
                    if (
                        ...
                        console.warn(
                            `[GraphQL] Network error for deploy ${deployId} (server may be down). Using fallback method.`,
                        );
                        return this.waitForDeployResultFallback(deployId);
                    }
                    ...
                    if (error.message.includes("CORS")) {
                        console.warn(
                            `[GraphQL] CORS error for deploy ${deployId}. Using fallback method.`,
                        );
                        return this.waitForDeployResultFallback(deployId);
                    }

                    console.warn(
                        `[GraphQL] Unexpected error for deploy ${deployId}. Trying fallback method.`,
                    );
                    return this.waitForDeployResultFallback(deployId);
                }
                ...
                if (
                    response.data?.data?.deployments &&
        ...
        console.warn(
            `[GraphQL] Deploy ${deployId} not found after ${maxAttempts} attempts (${maxAttempts * 5} seconds). The deploy may still be processing.`,
        );
    ...
    private async waitForDeployResultFallback(deployId: string): Promise<any> {
        ...
        try {
            ...
            if (blocksResult.data && Array.isArray(blocksResult.data)) {
                for (const block of blocksResult.data) {
                    if (block.deploys && Array.isArray(block.deploys)) {
                        const foundDeploy = block.deploys.find(
                            (deploy: any) =>
                                deploy.sig === deployId ||
                                deploy.signature === deployId ||
                                deploy.deployId === deployId,
                        );
                        ...
                        if (foundDeploy) {
                            ...
                            return {
                                status: "completed",
                                message:
                                    "Deploy successfully included in block",
            ...
            return {
                status: "pending",
                message:
                    "Deploy submitted successfully. Status check unavailable, but deploy may still be processing.",
                deployId: deployId,
            };
        } catch (fallbackError: any) {
            console.error(
                "[GraphQL] Fallback method also failed:",
                fallbackError,
            );
            ...
            return {
                status: "submitted",
                message:
                    "Deploy submitted successfully. Status check services are unavailable, but the deploy was sent to the network. Status will be checked automatically when services are available.",
                deployId: deployId,
            };
        }
    }
    ...
    async fetchTransactionHistory(
        ...
        if (!graphqlEndpoint) {
            console.error(
                "[GraphQL] No GraphQL endpoint configured. Current value:",
                graphqlEndpoint,
            );
        ...
        `,
            ...
            try {
                ...
                if (
                    ...
                    console.warn(
                        "[GraphQL] CORS or network error detected. Transaction history will be empty until API is configured properly.",
                    );
                    return [];
                }
            ...
            const transferTxs = transfers.map((tx: any) => {
                ...
                const normalizedFromAddress = tx.from_address
                    ?.toLowerCase()
                    .trim();

                const isReceive =
                    normalizedToAddress &&
                    normalizedToAddress === normalizedAddress;
                const isSend =
            ...
            const sortedTxs = Array.from(txMap.values()).sort(
                (a, b) =>
    ...
    async isNodeAccessible(
        nodeType: "validator" | "readOnly" | "admin" = "validator",
    ): Promise<boolean> {
        try {
            let client: AxiosInstance;
    ...
    async propose(): Promise<any> {
        if (!this.adminClient) {
            throw new Error(
                "Admin URL not configured. Propose is only available for local networks.",
            );
        }
