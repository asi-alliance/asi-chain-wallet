...
export class AccountsVault {
  ...
  private static persistAsync(): void {
    ...
    if (adapter) {
      adapter.putAccounts(records).catch((err: unknown) => {
        console.error('[AccountsVault] IDB write failed, using localStorage fallback:', err);
  ...
  private static persistToLocalStorage(): void {
    try {
      ...
      const parsed = current ? (JSON.parse(current) as LegacyStorageShape) : {};
      localStorage.setItem(LS_STORAGE_KEY, JSON.stringify({ ...parsed, accounts: this.accounts }));
  ...
  static async saveAccount(account: Account, password: string, userId: string): Promise<SecureAccount> {
    ...
    const secureAccount: SecureAccount = { ...rest, encryptedPrivateKey, userId };

    const existing = this.accounts.findIndex(a => a.id === account.id);
    const updated = [...this.accounts];
    if (existing >= 0) { updated[existing] = secureAccount; } else { updated.push(secureAccount); }
    this.save(updated);
    await this.flush();
  ...
  static async unlockAccount(accountId: string, password: string, userId: string): Promise<Account | null> {
    ...
    if (!this.validateOwnership(secureAccount, password, userId)) return null;
  ...
  private static validateOwnership(account: SecureAccount, password: string, userId: string): boolean {
    if (account.userId && account.userId !== userId) return false;
    if (!account.name) return true;
    return AccountsVault.generateUserId(password, account.name) === (account.userId ?? userId);
  }

  private static async decrypt(encrypted: string, password: string, version: PayloadVersion): Promise<string | undefined> {
    if (version === PayloadVersion.V2) {
      try { return await openV2(encrypted, password); } catch { return undefined; }
    }
    return legacyDecrypt(encrypted, password);
  }

  // Re-encrypts V1 payload to V2 format; async, retries on next unlock
  private static async reEncryptToV2(accountId: string, privateKey: string, password: string): Promise<void> {
    try {
      const encrypted = await sealV2(privateKey, password);
      this.save(this.accounts.map(a => a.id === accountId ? { ...a, encryptedPrivateKey: encrypted } : a));
    } catch { /* retry on next unlock */ }
  }
  ...
  static export(accountId: string): string | null {
    ...
    return JSON.stringify({
      version: 1, type: 'asi-wallet-keyfile',
      address: account.address, revAddress: account.revAddress,
      ethAddress: account.ethAddress, encryptedPrivateKey: account.encryptedPrivateKey,
      timestamp: new Date().toISOString(),
    }, null, 2);
  }

  static exists(revAddress?: string, ethAddress?: string, userId?: string): boolean {
    ...
    return pool.some(a => {
      ...
      return (rev && aRev && rev === aRev) || (eth && aEth && eth === aEth);
    });
  }

  static async importFromKeyfile(keyfileContent: string, name: string, userId: string, networkId?: string): Promise<SecureAccount> {
    let data: KeyfileData;
    try { data = JSON.parse(keyfileContent) as KeyfileData; }
    catch { throw new Error('Failed to import keyfile: Invalid format'); }

    if (data.type !== 'asi-wallet-keyfile') throw new Error('Invalid keyfile format');

    const account: SecureAccount = {
      id: Date.now().toString(), name,
      address: data.address ?? data.revAddress ?? '',
      revAddress: data.revAddress ?? '', ethAddress: data.ethAddress ?? '',
      publicKey: '', encryptedPrivateKey: data.encryptedPrivateKey,
      balance: '0', userId,
      ...(networkId ? { networkId } : {}),
      createdAt: new Date(),
    };
  ...
  static updateNetworks(updateMap: Map<string, string>): boolean {
    ...
    const updated = this.accounts.map(a => {
      ...
      if (next && a.networkId !== next) { changed = true; return { ...a, networkId: next }; }
      return a;
    });
