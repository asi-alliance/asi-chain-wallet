import { normalizeUint8Array } from "./../utils/helpers";
...
export class BiometricAuthService {
    ...
    public static async getCapabilities(): Promise<BiometricCapabilities> {
        ...
        try {
            ...
            return {
                ...
                isAvailable:
                    platformAuthenticatorAvailable ||
                    supportedAuthenticators.length > 0,
                supportedAuthenticators,
                platformAuthenticatorAvailable,
                conditionalMediationAvailable,
            };
    ...
    public static async registerCredential(
        ...
        try {
            ...
            const biometricCredential: BiometricCredential = {
                ...
                counter: response.getAuthenticatorData
                    ? this.parseCounter(response.getAuthenticatorData())
                    : 0,
                transports:
    ...
    public static async authenticate(
        ...
        try {
            ...
            if (!challenge) {
                crypto.getRandomValues(normalizeUint8Array(authChallenge));
            }
            ...
            const publicKeyCredentialRequestOptions: PublicKeyCredentialRequestOptions =
                {
                    challenge: normalizeUint8Array(authChallenge),
                    allowCredentials,
                    timeout: 60000,
                    userVerification: "required",
                    rpId: this.RP_ID,
            ...
            if (response.authenticatorData) {
                credential.counter = this.parseCounter(
    ...
    public static async getRemainingLockoutTime(): Promise<number> {
        ...
        if (
            settings.failedAttempts < this.MAX_RETRIES ||
    ...
    public static async disable(): Promise<{
        success: boolean;
        error?: string;
    }> {
        try {
            ...
            if (settingsData) {
                ...
                parsed.credentials.forEach((cred: any) => {
                    ...
                    if (typeof cred.publicKey === "string") {
                        cred.publicKey = this.base64ToArrayBuffer(
                            cred.publicKey,
                        );
                    }
                });
    ...
    public static async testBiometricAvailability(): Promise<{
        available: boolean;
        error?: string;
    }> {
        try {
            ...
            if (!capabilities.isAvailable) {
                return {
                    available: false,
                    error: "No biometric authenticators available",
                };
            }
            ...
            return {
                available: false,
                error: "Failed to test biometric availability",
            };
        }
    }
}
