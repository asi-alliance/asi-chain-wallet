...
export const createMultisigWallet = createAsyncThunk(
    ...
    async (request: CreateMultisigWalletRequest, { rejectWithValue }) => {
        try {
            const wallet =
                await multisigWalletService.createMultisigWallet(request);
            return wallet;
        } catch (error) {
            return rejectWithValue(
                error instanceof Error
    ...
    async (
        {
            walletId,
            deployRequest,
        }: { walletId: string; deployRequest?: DeployMultisigRequest },
        { rejectWithValue },
    ) => {
        try {
            const contractAddress =
                await multisigWalletService.deployMultisigWallet(
                    walletId,
                    deployRequest,
                );
            const updatedWallet =
                await multisigWalletService.getWallet(walletId);
    ...
    async (walletId: string, { rejectWithValue }) => {
        try {
            ...
            if (wallet.contractAddress) {
                balances = await multisigWalletService.getWalletBalance(
...
export const signMultisigTransaction = createAsyncThunk(
    ...
    async (transactionId: string, { rejectWithValue }) => {
        try {
            const signature =
                await multisigWalletService.signTransaction(transactionId);
...
const multisigSlice = createSlice({
    ...
    reducers: {
        selectWallet: (
            state,
            action: PayloadAction<MultisigWalletConfig | null>,
        ) => {
        ...
        updateTransaction: (
            state,
            action: PayloadAction<MultisigTransaction>,
        ) => {
        ...
        builder
            ...
            .addCase(deployMultisigWallet.fulfilled, (state, action) => {
                ...
                if (action.payload.wallet) {
                    const index = state.wallets.findIndex(
                        (w) => w.id === action.payload.wallet!.id,
                    );
                    if (index >= 0) {
            ...
            .addCase(signMultisigTransaction.fulfilled, (state, _action) => {
            ...
            .addCase(executeMultisigTransaction.fulfilled, (state, action) => {
                ...
                if (txIndex >= 0) {
                    ...
                    state.transactions[txIndex].transactionHash =
...
// Selectors
export const selectMultisigState = (state: { multisig: MultisigState }) =>
    state.multisig;

export const selectMultisigWallets = (state: { multisig: MultisigState }) =>
    state.multisig.wallets;

export const selectSelectedMultisigWallet = (state: {
    multisig: MultisigState;
}) => state.multisig.selectedWallet;

export const selectMultisigTransactions = (state: {
    multisig: MultisigState;
}) => state.multisig.transactions;

export const selectPendingMultisigTransactions = (state: {
    multisig: MultisigState;
}) =>
    state.multisig.transactions.filter(
        (tx) => tx.status === "pending" || tx.status === "approved",
    );

export const selectMultisigActivities = (state: { multisig: MultisigState }) =>
    state.multisig.activities;

export const selectMultisigBalances = (state: { multisig: MultisigState }) =>
    state.multisig.balances;

export const selectMultisigError = (state: { multisig: MultisigState }) =>
    state.multisig.error;

export const selectIsMultisigLoading = (state: { multisig: MultisigState }) =>
    state.multisig.isLoading;

export const selectIsCreatingMultisig = (state: { multisig: MultisigState }) =>
    state.multisig.isCreating;

export const selectIsDeployingMultisig = (state: { multisig: MultisigState }) =>
    state.multisig.isDeploying;
